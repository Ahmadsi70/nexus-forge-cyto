...
//!
//! **Input**: Set the `CYTO_MONUSEG_XML` env var to the MoNuSeg XML path to override the default.
...
/// Canonical MoNuSeg regression XML (override via CYTO_MONUSEG_XML env var — see [`resolved_monuseg_xml_path`]).
...
fn run_python_gold_gen(xml: &Path, gold_out: &Path) {
...
fn monuseg_tcga_fg_a4mu_xml_to_enriched_json() {
    ...
    let rows = nexus_forge_cyto::write_enriched_geometric_output(&raw, &enriched_path)
