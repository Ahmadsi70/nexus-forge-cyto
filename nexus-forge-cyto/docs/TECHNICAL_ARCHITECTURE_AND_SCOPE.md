...
**Nexus-Forge Cyto** is a **deterministic**, **geometry-first** cytology analytics engine: expert or dataset-derived **closed polygons** are normalized to fixed **`(128 × 2)`** vertex rings, enriched with **pure Rust** morphometrics and spatial topology metrics, scored by a **transparent weighted heuristic** (no trained neural model), augmented with **convex-hull interface distances**, and exported for **QuPath** measurement tables. Optional linkage to a **Modular Mojo** shared library (`math_core`) provides curvature κ on a **64-pt** downsample; when that library is **not** linked, a **documented fallback** keeps the pipeline runnable with **κ = 0**.
...
|--------|----------------|
| **In scope** | Parsing **MoNuSeg / ImageScope XML** (`Regions` → `Vertices`), **polygon JSON**, and **CSV vertex tables**; resampling each polygon to exactly **`TARGET_RING = 128`** points in **`(x, y)`**; emitting the segment envelope consumed by Rust (`cell_count`, `cells`, metadata). |
| **Primary artifacts** | **`services/nexus_core/coord_adapter.py`**: `parse_monuseg_xml`, `ring_from_polygon_xy`, `segment_envelope_from_polygons`, `write_gold_segment_json`; helper **`tests/validation/gen_gold_from_xml.py`** for CLI-style XML → JSON. |
| **Rust contract** | **`write_enriched_geometric_output`** in **`geometric_orchestrator.rs`**: consumes gold segment JSON (`cell_count`, `cells` with **128** vertices per cell) and emits enriched morphometric + clinical JSON. |
...
|--------|--------|----------------|
| **Area** | Shoelace / Green’s theorem on the **128-vertex ring** (absolute area). | **`spatial.rs`** — `polygon_area_shoelace_abs` |
...
| **Eccentricity** | Population covariance of the **128** boundary samples as a **2×2** matrix; eigenvalues \(\lambda_1 \ge \lambda_2\); \(e = \sqrt{\max(0,\,1-\lambda_2/\lambda_1)}\). | **`polygon_eccentricity`** |
...
| **FFI / mmap pipeline** (`pipeline.rs`, `pipeline_parallel.rs`) | Separate **dense `(64, D)` kappa work buffer** path for κ via Mojo when linked — distinct from segment JSON enrichment (128-pt rings downsampled to 64) but shares **`CytoNeighborMatrixView`** / status taxonomy. |
