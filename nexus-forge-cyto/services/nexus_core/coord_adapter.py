# -*- coding: utf-8 -*-
"""
Gold-standard annotation adapter: MoNuSeg XML, PanNuke-like JSON, **CSV vertex tables** → **TARGET_RING×(x,y)** rings
(**why**: locked vertex count for Rust κ downsample 128→64 — using **only** human-provided boundaries).
...
def ring_from_polygon_xy(points: list[tuple[float, float]]) -> list[list[float]]:
...
# Back-compat alias used by tests and legacy imports.
ring32_from_polygon_xy = ring_from_polygon_xy
...
def segment_envelope_from_polygons(
    ...
    **`gold_standard_source`**: when **`"CSV-Gold-Standard"`**, adds provenance + interpretation hints for auditors.
    ...
    for poly in polygons:
        rings32.append(ring_from_polygon_xy(list(poly)))
