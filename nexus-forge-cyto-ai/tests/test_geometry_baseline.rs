//! Geometric smoke tests: deterministic `(32, 2)` tensors through mmap + optional Mojo κ core.
//!
//! **Why two tiers**: Default CI builds omit `CYTO_MOJO_LIB_DIR`; layout/session tests always run.
//! The κ-vs-shape baseline requires `CYTO_MOJO_LIB_DIR` and `LD_LIBRARY_PATH` linking `libmath_core.so`;
//! CI may skip by not setting those vars (suite returns `CytoErrMojoLibraryUnset` for callers).
...
fn geometry_tempfile_layout_and_pipeline_invocation_smoke() {
    ...
    match res {
        Ok((kappa, clinical)) => {
            assert!(kappa.is_finite(), "κ must be finite when Mojo returns Ok");
            assert!(
                matches!(
                    clinical,
        ...
        Err(CytoStatus::CytoErrMojoLibraryUnset) => {
            // Expected when `CYTO_MOJO_LIB_DIR` was not set at build time.
        }
...
/// Runs only when Phase-3 links `math_core`; otherwise skips with actionable message (no panic).
#[test]
fn geometry_kappa_baseline_circle_vs_spiked_star() {
    ...
    let circle = match analyze_slide_with_dims(tmp_a.path().to_str().unwrap(), D, 1.0_f32) {
        Ok(v) => v,
        Err(CytoStatus::CytoErrMojoLibraryUnset) => {
            eprintln!(
                "skip geometry_kappa_baseline_circle_vs_spiked_star: CYTO_MOJO_LIB_DIR absent at compile time"
            );
            return;
        }
        Err(e) => panic!("Phase-3 / Mojo: {:?}", e),
    };
    let star = match analyze_slide_with_dims(tmp_b.path().to_str().unwrap(), D, 1.0_f32) {
        Ok(v) => v,
        Err(CytoStatus::CytoErrMojoLibraryUnset) => return,
        Err(e) => panic!("Phase-3 / Mojo: {:?}", e),
    };
