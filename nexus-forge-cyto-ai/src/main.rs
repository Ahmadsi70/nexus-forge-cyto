...
use nexus_forge_cyto_ai::write_enriched_geometric_output;
...
struct Cli {
    ...
    /// Optional slide image to extract vision features from.
    #[arg(long = "image")]
...
fn process_one(input_path: &Path, input_root: &Path, output_dir: &Path, image_path: Option<&Path>) -> Result<(), String> {
    ...
    write_enriched_geometric_output(&raw, &dest, image_path)
...
fn main() {
    ...
    let image_ref = cli.image.as_deref();

    paths.par_iter().for_each(
        |input_path| match process_one(input_path, &input_root, &cli.output_dir, image_ref) {
            Ok(()) => {
